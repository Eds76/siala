...
$sagraClasses = "barbarian","fighter","weaponmaster","dwarvendefender"
...
{	
	param ($__comboBox,$__slider,$__tb_sld,$__skills,$__HP)
	$__comboBox.add_SelectionChanged(
	{
		set_default
		if($this.selecteditem -in $eClasses)
	...
	$__slider.add_valuechanged(
	{
		...
		$tb_SKILLS_INT.text = [int]$tb_lvlSum_CR.text * [math]::Floor(([int]$tb_INT_START.text - 10)/2) + ([math]::Floor(([int]$tb_INT_START.text - 10)/2)) * 4
...
{
	slider_combobox2 $cmb_class1_CR $sld_class1_CR $tb_Sld1_CR $tb_SKILLS1 $tb_HP1
	slider_combobox2 $cmb_class2_CR $sld_class2_CR $tb_Sld2_CR $tb_SKILLS2 $tb_HP2
	slider_combobox2 $cmb_class3_CR $sld_class3_CR $tb_Sld3_CR $tb_SKILLS3 $tb_HP3
	slider_combobox2 $cmb_class4_CR $sld_class4_CR $tb_Sld4_CR $tb_SKILLS4 $tb_HP4
}
SldCMb_calculation2
#####################################
...
{
	if(($comboboxes_CR[0].selectedvalue -in $sagraclasses ))
